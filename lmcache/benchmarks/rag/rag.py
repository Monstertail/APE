...
class WorkloadConfig:
    ...
    max_tokens: int
    # Output file path for prompts and generated text in jsonl format
    output_jsonl: str
...
class RAGManager:
    ...
    def __init__(self, workload_config: WorkloadConfig):
        ...
        self._ttft = []
        self._tpot = []
        # Store output jsonl path
        self._output_jsonl = workload_config.output_jsonl
    ...
    def _save_to_jsonl(self, request_id: int, launch_time: float, generated_text: str):
        """Save prompt and generated text to jsonl file.
        
        Args:
            request_id: The ID of the request
            launch_time: The timestamp when the request was launched
            generated_text: The text generated by the model
        """
        if not self._output_jsonl:
            return
        ...
        with open(self._output_jsonl, 'a', encoding='utf-8') as f:
            data = {
                ...
                'ground_truth': self._answers[request_id],
            }
    ...
    def _update_result(self, response: Response):
        ...
        # Save to jsonl if output path is specified
        if self._output_jsonl:
            self._save_to_jsonl(response.request_id, response.launch_time, response.body)
...
def run_rag(args):
    ...
    workload_config = WorkloadConfig(
        ...
        output_jsonl=args.output_jsonl  # Add output_jsonl parameter
    )
